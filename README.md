...
> Memory only affects behaviour if `principles_prompt` is consumed by your agent's `act()`. See `examples/quickstart.py` for a self-contained demo with a principle-aware policy and a visible learning curve.
